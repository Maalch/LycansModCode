...
public class MolotovItem : CustomItem
{
	private static Changed<MolotovItem> _0024IL2CPP_CHANGED;

	private static ChangedDelegate<MolotovItem> _0024IL2CPP_CHANGED_DELEGATE;

	private static NetworkBehaviourCallbacks<MolotovItem> _0024IL2CPP_NETWORK_BEHAVIOUR_CALLBACKS;

	public override string PrefabName => "LycansNewRoles.ItemMolotove";
	...
	protected override void ItemTriggered()
	{
		...
		Rpc_Use_Molotov(((SimulationBehaviour)this).Runner, ((Item)this).Owner, ((Component)player.PlayerController.LocalCameraHandler.LocalCamera).transform.forward.y);
	}

	[Rpc]
	public unsafe static void Rpc_Use_Molotov(NetworkRunner runner, PlayerRef playerRef, float angleY)
	{
		...
		try
		{
			...
			else
			{
				...
				if (runner.HasAnyActiveConnections())
				{
					...
					int num2 = RpcHeader.Write(RpcHeader.Create(NetworkBehaviourUtils.GetRpcStaticIndexOrThrow("System.Void LycansNewRoles.MolotovItem::Rpc_Use_Molotov(Fusion.NetworkRunner,Fusion.PlayerRef,System.Single)")), data);
			...
			if (!(item is MolotovItem molotovItem))
			{
				return;
			}
			TickTimer itemTimer = ((Item)molotovItem).ItemTimer;
			if (!((TickTimer)(ref itemTimer)).IsRunning)
			{
				((Item)molotovItem).ItemQuantity = ((Item)molotovItem).ItemQuantity - 1;
				if (((Item)molotovItem).ItemQuantity > 0)
				{
					((Item)molotovItem).StartDelayTimer(Traverse.Create((object)molotovItem).Field<float>("resetDelay").Value);
				}
				else
				{
					((Item)molotovItem).DestroyItem();
				}
				NetworkPrefabId networkObject = NetworkObjectService.Instance.GetNetworkObject("LycansNewRoles.ItemMolotovActive");
				...
				if (player.Stats != null)
				{
					player.Stats.AddAction(new PlayerStats.PlayerAction
					{
						...
						ActionName = TranslationManager.Instance.GetTranslation("NALES_ITEM_MOLOTOV")
		...
		{
			Plugin.Logger.LogError((object)("Rpc_Use_Molotov error: " + ex));
		}
	}

	[NetworkRpcStaticWeavedInvoker("System.Void LycansNewRoles.MolotovItem::Rpc_Use_Molotov(Fusion.NetworkRunner,Fusion.PlayerRef,System.Single)")]
	[Preserve]
	protected unsafe static void Rpc_Use_Molotov_0040Invoker(NetworkRunner runner, SimulationMessage* message)
	{
		...
		Rpc_Use_Molotov(runner, playerRef, angleY);
	}
