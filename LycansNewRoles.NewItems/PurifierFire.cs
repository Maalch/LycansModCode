...
public class PurifierFire : NetworkBehaviour
{
	...
	public unsafe int DurationMilliseconds
	{
		get
		{
			if (base.Ptr == null)
			{
				throw new InvalidOperationException("Error when accessing PurifierFire.DurationMilliseconds. Networked properties can only be accessed when Spawned() has been called.");
			}
			return *base.Ptr;
		}
		set
			...
			if (LycansUtility.GameActuallyInPlay)
			{
				...
				foreach (PlayerCustom item in enumerable)
				{
					...
					if (!((TickTimer)(ref value)).IsRunning)
					{
						...
						{
							PlayerCustom.ApplyEffectToPlayer(item.PlayerController, "LycansNewRoles.EffectConfused", ((SimulationBehaviour)this).Runner, 1f, 3f);
						}
					}
				}
			}
