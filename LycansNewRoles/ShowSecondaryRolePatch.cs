...
[HarmonyPatch(typeof(GameUI), "UpdateRole")]
internal class ShowSecondaryRolePatch
{
	private static bool Prefix(PlayerRole playerRole, GameUI __instance)
	{
		...
		try
		{
			...
			if (!PlayerCustomRegistry.HasPlayer(PlayerController.Local.Ref))
			{
				return true;
			}
			PlayerCustom player = PlayerCustomRegistry.GetPlayer(PlayerController.Local.Ref);
			RoleDescription fullRoleDescription = UpdateRoleUtility.GetFullRoleDescription(PlayerController.Local, player, withAlly: false);
		...
		{
			Plugin.Logger.LogError((object)("Postfix UpdateRole error: " + ex));
			return true;
		}
	}
}
