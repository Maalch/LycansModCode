...
public static class UpdateRoleUtility
{
	public static Color GetPrimaryRoleColor(PlayerRole playerRole, PlayerCustom.PlayerNewPrimaryRole newPrimaryRole, PlayerCustom.PlayerPrimaryRolePower primaryRolePower)
	{
		...
		{
		case 0:
			result = (((uint)(primaryRolePower - 27) > 1u) ? GameUI.VillagerColor : PlayerCustom.GetPrimaryRolePowerColor(primaryRolePower));
			break;
		case 1:
	...
	public static string GetPrimaryRoleDetails(PlayerCustom playerCustom, bool forSpectator)
	{
		...
		case PlayerCustom.PlayerNewPrimaryRole.Cultist:
			...
			if (playerCustom.PrimaryRolePowerRemainingUses > 0)
			{
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_CULTIST_AVAILABLE").Replace("{0}", Mathf.FloorToInt((float)(playerCustom.SoloRoleObjectiveCount * 100 / 10000)).ToString()).Replace("{1}", CultistSkull.AllSkulls.Count.ToString());
			}
			primaryRolePowerCooldownTimer = playerCustom.PrimaryRolePowerCooldownTimer;
			if (((TickTimer)(ref primaryRolePowerCooldownTimer)).IsRunning)
			{
		...
		default:
			...
			{
				int num2 = Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials));
				if (!NetworkBool.op_Implicit(playerCustom.NewPrimaryRoleUniqueBool))
				{
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_NECROMANCER_NO_TARGET").Replace("{0}", Mathf.FloorToInt((float)num2).ToString());
				}
				string text3 = ((playerCustom.PrimaryRolePowerCurrentMaterials >= 10000) ? "<color=green>" : "<color=red>");
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_NECROMANCER_TARGET").Replace("{0}", text3 + Mathf.FloorToInt((float)num2) + "</color>");
			}
			case PlayerCustom.PlayerPrimaryRolePower.Deceiver:
				...
				if (((TickTimer)(ref primaryRolePowerCooldownTimer)).IsRunning)
				{
					string translation = TranslationManager.Instance.GetTranslation("NALES_ROLE_DECEIVER_AVAILABLE_IN");
					primaryRolePowerCooldownTimer = playerCustom.PrimaryRolePowerCooldownTimer;
			...
			case PlayerCustom.PlayerPrimaryRolePower.Warlock:
				...
				if (((TickTimer)(ref primaryRolePowerCooldownTimer)).IsRunning)
				{
					string translation2 = TranslationManager.Instance.GetTranslation("NALES_ROLE_WARLOCK_AVAILABLE_IN");
					primaryRolePowerCooldownTimer = playerCustom.PrimaryRolePowerCooldownTimer;
			...
			case PlayerCustom.PlayerPrimaryRolePower.Possessor:
			{
				...
				if (playerCustom.PrimaryRolePowerCurrentMaterials < playerCustom.PowerMaterialsInfo.RequiredMaterials)
				{
					if (NetworkBool.op_Implicit(playerCustom.NewPrimaryRoleUniqueBool))
					{
						return TranslationManager.Instance.GetTranslation("NALES_ROLE_POSSESSOR_TARGET_VISIBLE").Replace("{0}", ((object)player2.PlayerController.PlayerData.Username/*cast due to constrained. prefix*/).ToString()).Replace("{1}", Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)).ToString());
					}
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_POSSESSOR_TARGET_NOT_VISIBLE").Replace("{0}", ((object)player2.PlayerController.PlayerData.Username/*cast due to constrained. prefix*/).ToString());
				}
				if (NetworkBool.op_Implicit(playerCustom.NewPrimaryRoleUniqueBool) || (NetworkBool.op_Implicit(playerCustom.PlayerController.IsWolf) && NetworkBool.op_Implicit(player2.Possessed)))
				{
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_POSSESSOR_POSSESSING").Replace("{0}", ((object)player2.PlayerController.PlayerData.Username/*cast due to constrained. prefix*/).ToString());
				}
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_POSSESSOR_AVAILABLE").Replace("{0}", ((object)player2.PlayerController.PlayerData.Username/*cast due to constrained. prefix*/).ToString());
			}
			...
			case PlayerCustom.PlayerPrimaryRolePower.Tracker:
				if (NetworkBool.op_Implicit(playerCustom.NewPrimaryRoleUniqueBool))
				{
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_TRACKER_ACTIVE").Replace("{0}", LycansUtility.GetInputDisplayCustom((InputActionName)5));
				}
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_TRACKER_INACTIVE").Replace("{0}", LycansUtility.GetInputDisplayCustom((InputActionName)5));
			...
			{
				string text = ((playerCustom.PrimaryRolePowerCurrentMaterials >= playerCustom.PowerMaterialsInfo.RequiredMaterials) ? "<color=green>" : "<color=red>");
				int count = MagicianBeacon.AssociatedBeacons.Count;
				string text2 = ((count > 0) ? "<color=green>" : "<color=red>");
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_MAGICIAN_DETAILS").Replace("{0}", text + Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)) + "%</color>").Replace("{1}", text2 + count + "</color>");
			}
			...
			case PlayerCustom.PlayerPrimaryRolePower.Runemaster:
				if (playerCustom.PrimaryRolePowerRemainingUses > 0)
				{
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_RUNEMASTER_DETAILS").Replace("{0}", "<color=green>" + Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)) + "%</color>").Replace("{1}", (playerCustom.Ref == PlayerController.Local.Ref) ? playerCustom.AssociatedRunes.Count.ToString() : "?")
						.Replace("{2}", 6.ToString());
				}
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_RUNEMASTER_DETAILS").Replace("{0}", "<color=red>" + Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)) + "%</color>").Replace("{1}", (playerCustom.Ref == PlayerController.Local.Ref) ? playerCustom.AssociatedRunes.Count.ToString() : "?")
					.Replace("{2}", 6.ToString());
			case PlayerCustom.PlayerPrimaryRolePower.Inventor:
				if (playerCustom.PrimaryRolePowerCurrentMaterials >= playerCustom.PowerMaterialsInfo.RequiredMaterials)
				{
					return TranslationManager.Instance.GetTranslation("NALES_ROLE_INVENTOR_DETAILS").Replace("{0}", "<color=green>" + Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)) + "%</color>");
				}
				return TranslationManager.Instance.GetTranslation("NALES_ROLE_INVENTOR_DETAILS").Replace("{0}", "<color=red>" + Mathf.FloorToInt((float)(playerCustom.PrimaryRolePowerCurrentMaterials * 100 / playerCustom.PowerMaterialsInfo.RequiredMaterials)) + "%</color>");
	...
	public static RoleDescription GetFullRoleDescription(PlayerController player, PlayerCustom playerCustom, bool withAlly)
	{
		...
		if (withAlly && newPrimaryRole != PlayerCustom.PlayerNewPrimaryRole.Lover && (newPrimaryRole == PlayerCustom.PlayerNewPrimaryRole.Traitor || PlayerCustomRegistry.CountWhere((PlayerCustom o) => (int)o.PlayerController.Role == 1 && o.NewPrimaryRole != PlayerCustom.PlayerNewPrimaryRole.Lover) > 1))
		{
			...
			List<PlayerRef> list2 = new List<PlayerRef> { player.Ref, item };
			if (newPrimaryRole == PlayerCustom.PlayerNewPrimaryRole.Traitor)
			{
				PlayerCustom playerCustom3 = PlayerCustomRegistry.AllPlayers.FirstOrDefault((PlayerCustom o) => o.PrimaryRolePower == PlayerCustom.PlayerPrimaryRolePower.Mole);
				PlayerRef item2 = (((Object)(object)playerCustom3 != (Object)null) ? playerCustom3.Ref : PlayerRef.None);
				list2.Add(item2);
			}
			string text2 = ListWolvesAsString(list2);
			string text3 = "";
