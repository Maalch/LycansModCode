...
public class GameManagerCustom : NetworkBehaviour
{
	...
	public void NewDay()
	{
		...
		CurrentDay++;
		LycansUtility.AddLogOnlyForMe("Stats: NewDay now -> " + CurrentDay);
		if (CurrentDay > 1)
		{
	...
	public override void FixedUpdateNetwork()
	{
		...
		//IL_1096: Invalid comparison between Unknown and I4
		...
		foreach (PlayerCustom playerCustom in enumerable)
		{
			...
			if (LycansUtility.GameActuallyInPlay)
			{
				...
				case PlayerCustom.PlayerNewPrimaryRole.Cultist:
				{
					...
					if (count > 0)
					{
						float num5 = BalancingValues.CultistChargeGainForSkullsCount(count) * ((float)Plugin.CustomConfig.CultistSpeed * 0.01f) * BalancingValues.CultistChargeGainMultiplierForPlayersAmount(PlayerRegistry.CountWhere((Predicate<PlayerController>)((PlayerController o) => !NetworkBool.op_Implicit(o.IsDead))));
						num5 /= SoloRoleDifficulty;
						playerCustom.AddSoloRoleProgress(Mathf.RoundToInt(num5), 10000);
					}
					break;
				}
				}
				...
				case PlayerCustom.PlayerPrimaryRolePower.Shadow:
				{
					...
					playerCustom.AddMaterials(Mathf.RoundToInt(num24));
					break;
				}
				case PlayerCustom.PlayerPrimaryRolePower.Necromancer:
				{
					...
					if (!((PlayerRef)(ref primaryRoleTargetRef)).IsNone && !NetworkBool.op_Implicit(PlayerRegistry.GetPlayer(playerCustom.PrimaryRoleTargetRef).IsDead))
					{
						...
						if (playerCustom.PrimaryRolePowerCurrentMaterials == 0 || NetworkBool.op_Implicit(playerCustom.PlayerController.IsDead))
						{
				...
				case PlayerCustom.PlayerPrimaryRolePower.Possessor:
				{
					...
					float num23 = BalancingValues.PossessorMaximumRangeByMap(GameManager.Instance.MapID);
					if (Vector3.Distance(((Component)player).transform.position, ((Component)player3.PlayerController).transform.position) <= num23)
					{
						playerCustom.NewPrimaryRoleUniqueBool = NetworkBool.op_Implicit(true);
	...
	public unsafe static void Rpc_Mayor_Action(NetworkRunner runner, int playerIndex, int targetPlayerIndex, int actionIndex)
	{
		...
		case 1:
			...
			{
				UIManager.ShowRedCenterMessage("NALES_UI_MAYOR_SPEECH_OTHER", 0.5f, 4f, new List<object> { ((object)targetPlayerCustom.PlayerController.PlayerData.Username/*cast due to constrained. prefix*/).ToString() });
			}
	...
	public unsafe static void Rpc_Ask_For_Speech(NetworkRunner runner, int playerIndex)
	{
		...
		if (!player.AskForSpeechActive && !player.AskForSpeechUsedThisMeeting && !(Instance.CurrentMayor == player.Ref))
		{
			...
			AudioManager.PlayPosition("VILLAGER", ((Component)player.PlayerController).transform.position, (MixerTarget)2, 50f, 0.5f);
		}
	}
