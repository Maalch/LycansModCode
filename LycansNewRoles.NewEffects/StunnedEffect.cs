...
public class StunnedEffect : CustomEffect
{
	public override string CustomEffectName => "LycansNewRoles.EffectStunned";

	public override string TranslateKey => "NALES_EFFECT_STUN";
	...
	protected override void ApplyEffectToPlayerSpecific(PlayerRef targetPlayer)
	{
		...
		{
			PlayerCustomRegistry.GetPlayer(targetPlayer).Stunned = NetworkBool.op_Implicit(true);
			if (NetworkBool.op_Implicit(player.IsAiming))
			{
	...
	protected override void RemoveEffectFromPlayerSpecific(PlayerRef targetPlayer)
	{
		...
		if (((SimulationBehaviour)this).HasStateAuthority)
		{
			...
			if ((Object)(object)player != (Object)null)
			{
				PlayerCustomRegistry.GetPlayer(targetPlayer).Stunned = NetworkBool.op_Implicit(false);
			}
		}
	}
}
